<template>
  <div class="serial-number-metric">
    <div class="dropdown">
...
const querySearch = debounce(async () => {
  ...
  if (!hasFilters) {
    const { data } = await http.get(`/api/search`, {
      params: { [`filter[serial_number]`]: state.input },
    });
    ...
    state.suggestions = products.value.data
      .filter(
        (x) =>
          x.serial_number &&
          x.serial_number.toLowerCase().startsWith(state.input.toLowerCase())
      )
      .map((y) => y.serial_number);
  }
});

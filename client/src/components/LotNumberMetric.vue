<template>
  <div class="lot-number-metric">
    <div class="dropdown">
      <form @submit.prevent="handleSubmit">
        <label for="lotNumberFilter" class="form-label">Lot Number</label>
...
const querySearch = debounce(async () => {
  ...
  if (!hasFilters) {
    const { data } = await http.get(`/api/search`, {
      params: { [`filter[lot_number]`]: state.input },
    });
    ...
    state.suggestions = products.value.data
      .filter(
        (x) =>
          x.lot_number &&
          x.lot_number.toLowerCase().startsWith(state.input.toLowerCase())
      )
      .map((y) => y.lot_number);
  }
}, 200);
